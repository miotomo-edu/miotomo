...
4. **Run** – `npm run dev` launches Vite on `http://localhost:5173`. Append `?transport=daily` to test the Daily SDK; append `?skipOnboarding=1` (or `&skipOnboarding=1` alongside other params) to bypass Landing/Onboarding and open directly on Library; append `?screenshotMode=1` to switch from the in-app mobile scroller to document-level page scrolling for easier full-page DevTools screenshots; the default path shows the full onboarding flow and uses Small WebRTC unless `transport=daily` is set.
