...
Keep Deepgram, Supabase, and Pipecat credentials in `.env.local` (consumed via `import.meta.env`). Never commit keys or raw child transcripts. Adjust defaults for `stsConfig` in `src/lib/constants.ts` and document any behavioral change here plus in `README.md` so every agent shares the same voice setup.
